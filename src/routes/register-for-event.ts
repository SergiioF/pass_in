...
export async function regiterForEvent(app: FastifyInstance) {
  app.withTypeProvider<ZodTypeProvider>().post(
    ...
    async (request, reply) => {
      ...
      const attendeeFromEmail = await prisma.attendee.findUnique({ where: { eventId_email: { email, eventId } } });

      if (attendeeFromEmail !== null) {
        throw new BadRequest("You're already registered for this event");
      }
